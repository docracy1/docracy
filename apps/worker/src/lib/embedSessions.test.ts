...
describe("embed sessions", () => {
  it("issues and resolves a session to a signing token when origin matches", async () => {
    ...
    const resolved = await resolveEmbedSession(env, issued.embedToken, "https://partner.example");
    expect(resolved).not.toBeNull();
    expect(resolved).not.toMatchObject({ error: "origin" });
  ...
  it("rejects resolve when parent origin is missing or mismatched", async () => {
    const { env } = makeMockEnv();
    await putDoc(env, makeDoc("acct-1"));
    const issued = await issueEmbedSession({
      env,
    ...
    expect(await resolveEmbedSession(env, issued.embedToken, null)).toEqual({ error: "origin", status: 403 });
    expect(await resolveEmbedSession(env, issued.embedToken, "https://evil.example")).toEqual({
      error: "origin",
      status: 403,
    });
  });
