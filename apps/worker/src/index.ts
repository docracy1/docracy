...
import remitWaitlistRoute from "./routes/remitWaitlist";
import transakRoute from "./routes/transak";
...
import { runCryptoReminderSweep, runPaymentFreezeSweep } from "./lib/paymentFreeze";
...
app.route("/api/remit-waitlist", remitWaitlistRoute);
app.route("/api/transak", transakRoute);
...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext) {
    ...
    ctx.waitUntil(runCryptoReminderSweep(env).catch((err) => console.error("Crypto reminder sweep failed:", err)));
  },
};
