 ...
 * Run from apps/web:
 *   node scripts/generateLegacyBatch.mjs          # batch 1
 *   node scripts/generateLegacyBatch.mjs --batch=2  # batch 2
 */
...
const batchNum = batchArg ? Number(batchArg.split("=")[1]) : 1;
const catalogFile =
  batchNum === 2
    ? path.join(__dirname, "legacyBatch/catalog-batch2.json")
    : path.join(__dirname, "legacyBatch/catalog.json");
const outTsFile =
  batchNum === 2
    ? path.join(__dirname, "..", "src", "lib", "freeTemplatesLegacyBatch2.ts")
    : path.join(__dirname, "..", "src", "lib", "freeTemplatesLegacyBatch.ts");
const fieldPrefix = batchNum === 2 ? "lb2" : "lb";
const exportName = batchNum === 2 ? "LEGACY_BATCH_2_TEMPLATES" : "LEGACY_BATCH_TEMPLATES";
const tsComment =
  batchNum === 2
    ? "Hand-curated docracy.com batch 2 (topic queue + URL clusters) — scripts/generateLegacyBatch.mjs --batch=2"
...
/** ${tsComment} */
export const ${exportName}: FreeTemplate[] = [
${tsEntries.join(",\n")}
];
`;

fs.writeFileSync(outTsFile, tsOut);
console.log(`\nWrote ${catalog.length} templates to ${path.basename(outTsFile)}`);
